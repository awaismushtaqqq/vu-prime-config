...
Challan Automation — Bulk extracts and marks paid fee challans on campus.vu.edu.pk
VU Pro Suite — Scrapes student data into the user's local database, provides a 360° student dashboard, search, and Excel export
WhatsApp Sender — Sends bulk educational reminder messages via web.whatsapp.com
Community Chat — Real-time group chat connecting verified VU Prime subscribers
...
Authentication — to verify your identity for premium feature access
License enforcement — to validate active premium subscriptions
Abuse prevention — to detect compromised accounts, device sharing beyond the allowed limit, and tampering
Service health — to detect bugs, crashes, and outages
Community chat moderation — to maintain a professional and safe communication environment
...
PermissionWhy we need itstorageTo save your session, settings, and locally-cached student datatabsTo coordinate automation across multiple browser tabsscriptingTo inject automation content scripts into authorized pagesalarmsTo run the license heartbeat check every 2 minutesidentityTo enable Google Sign-In via chrome.identity APInotificationsTo alert you when long-running jobs completeHost: campus.vu.edu.pkCore automation runs here (challans, student scraping, 360° view)Host: web.whatsapp.comWhatsApp Sender moduleHost: vu-prime-api.vu-prime.workers.devOur authentication server (license validation, chat)Host: googleapis.comGoogle OAuth profile fetchHost: worldtimeapi.orgFallback time source for anti-tampering
